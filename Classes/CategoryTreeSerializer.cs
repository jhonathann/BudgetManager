     ...
     public static void Deserialize(string jsonString)
     {
          ...
          foreach (KeyValuePair<string, JsonElement> categorieKvp in categories)
          {
               ...
               foreach (KeyValuePair<string, JsonElement> conceptKvp in concepts)
               {
                    ...
                    //Gets the concept Id
                    int conceptId = conceptKvp.Value.GetProperty("Id").GetInt32();
                    //Creates the concept
                    Concept concept = new(conceptKvp.Key, category, conceptId);
                    ...
                    foreach (KeyValuePair<string, JsonElement> rubroKvp in rubros)
                    {
                         ...
                         //Gets the rubro Id
                         int rubroId = rubroKvp.Value.GetProperty("Id").GetInt32();
                         //Creates the rubro
                         new Rubro(rubroKvp.Key, concept, rubroId);
                    }
               }
          }
     }
